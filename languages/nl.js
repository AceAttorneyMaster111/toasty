...
module.exports = class extends Language {
	constructor(...args) {
		super(...args);
		this.language = {
			DEFAULT: (key) => `${key} is nog niet gelocaliseerd (vertaald) voor nl.`,
      INHIBITOR_MISSING_BOT_PERMS: (missing) => `🚫 [**Ontbrekende Toestemmingen**]: Ik heb de **${missing}** Machtiging niet.`,
      INHIBITOR_PERMISSIONS: `🚫 [**Ongeldige Toestemmingen**]: Je hebt geen toestemming om dit commando te gebruiken.`,
      INHIBITOR_DISABLED: '🚫 Sorry, dit commando is momenteel uitgeschakeld',
			INHIBITOR_NSFW: '🚫 Je mag geen NSFW commando\'s in dit kanaal gebruiken.',
			...
			INHIBITOR_RUNIN: (types) => `🚫 Dit commando is alleen beschikbaar in **${types}** kanalen.`,
			INHIBITOR_RUNIN_NONE: (name) => `🚫 Het **${name}** commando is niet geconfigureerd om te worden uitgevoerd in een kanaal.`,
      COMMANDMESSAGE_MISSING: 'Één of meer verplichte argumenten missen na het einde van de input.',
			COMMANDMESSAGE_MISSING_REQUIRED: (name) => `**${name}** is een verplicht argument.`,

      // Bot Utility Commands
      INVITE_CMD: 'Je kunt mij uitnodigen op jouw server met de onderstaande link!\n**https://toastybot.com/invite**',
			INVITE_CMD_DESCRIPTION: 'Verstuurd de invite URL zodat je mij kan toevoegen op jouw server.',
			SHARDS_CMD_DESCRIPTION: 'Verstuurd statistieken naar de bot\'s shards.',
			STATS_CMD_DESCRIPTION: 'Verstuurd gedetailleerde statistieken over de bot.',

			// Miscellaneous Commands
			DONATE_CMD: 'Donaties zijn zeer gewaardeerd! Doneer met de onderstaande link.\n**https://toastybot.com/donate**',
			DONATE_CMD_DESCRIPTION: 'Steun Toasty door te doneren!',
			...
			POKEMON_CMD_EXTENDEDHELP: 'Laat je een willekeurige vangen elke 3 uur en slaat ze op in je virtuele inventaris.\nJe kan pokemons ruilen met andere spelers of ze zelf houden.',
			POKEMON_CMD_COOLDOWNMSG_1: 'Je moet nog',
			POKEMON_CMD_COOLDOWNMSG_2: 'wachten totdat je nog een pokemon kan vangen.',
			POKEMON_CMD_CATCHFAILMSG: 'het is je niet gelukt om een pokemon te vangen 😢',
			POKEMON_CMD_CATCHMSG: 'je ving een',

			INVENTORY_CMD_DESCRIPTION: 'Laat jouw, of de vermelde gebruiker\'s inventaris zien.',
