...
module.exports = class extends Command {
  constructor(...args) {
    super(...args, {
      ...
      extendedHelp: 'Catch pokemon with the pokemon command.\nYou can trade the pokemon you catch with this command.\n**Example Usage:** `trade @user pikachu charzard`',
      runIn: ['text'],
      enabled: false, //DISABLED BC OF ARGS
      /*args: [
        {
          key: 'user',
  ...
  async run(msg, [user, pokemon1, pokemon2]) {
    ...
    if (trading.includes(user.id)) return msg.sendMessage(`${user}, :no_entry_sign: Nice try but you cannot trade when you are already trading with someone!`);
    ...
    return msg.channel.awaitMessages(filter, { time: 30e3, errors: ['time'], max: 1 })
      .then(async collected => {
        ...
        if (['y', 'yes'].includes(collected.first().content.toLowerCase())) {
          ...
          await msg.sendMessage(`${user}, are you sure you want to trade that pokemon? Respond with \`yes\` or \`no\``);
