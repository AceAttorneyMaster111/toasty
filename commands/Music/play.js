...
module.exports = class extends Command {
    ...
    async run(msg, [song]) {
        ...
        if (this.client.validURL(song)) {
            ...
            if (playlist) {
                const songData = await this.client.getSongs(song);
                ...
                // Handling SoundCloud
                if (scPlaylist) {
                    const tracks = await this.client.getSongs(scPlaylist[0]);
                ...
                const songData = await this.client.getSongs(soundCloud[0]);
        ...
        /*} else if (song.match(/scsearch:.*//*) || song.match(/ytsearch:.*//*)) {
            ...
            // Search from YouTube
            const songsData = await this.client.getSongs(`ytsearch:${song}`);
            ...
            const selection = await msg.awaitReply([`?? | **Select a Song - PenguBot**\n`,
                `${options.map(o => `? \`${++index}\` ${o.info.title} - ${o.info.author} (${this.client.formatUptime(o.info.length)})`).join("\n")}`,
    ...
    async playEmbed(song) {
        return new MessageEmbed()
            ...
            .setDescription([`� **Song:** ${song.name}`,
                `� **Author:** ${song.author}`,
                `� **Length:** ${song.stream === true ? "Live Stream" : this.client.friendlyTime(song.length)}`,
