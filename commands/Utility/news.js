...
module.exports = class extends Command {
  constructor(...args) {
    super(...args, {
      ...
      usage: '<newsSource:string>',
      cooldown: 10
    });
  }

  async run(msg, [newsSource]) {
    const s = newsSource.toLowerCase();
    ...
    if (s === 'national-geographic') {
      source.name = 'national-geographic';
      source.icon = 'http://logok.org/wp-content/uploads/2014/06/National-Geographic-logo.png';
    } else {
      return msg.reply(`🚫 That's not a valid news source. Type, \`${msg.guildConfigs.prefix}sources\` to get a list of avaliable news sources.`);
    }
    ...
    embed.setColor(3447003)
      ...
      .setTitle(`📰 Latest news from ${source.name}:`)
