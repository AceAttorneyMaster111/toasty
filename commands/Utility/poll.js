...
module.exports = class extends Command {
  constructor(...args) {
    super(...args, {
      ...
      extendedHelp: '**Example Usage:** `;poll Minecraft or Roblox? | Minecraft | Roblox | Other`',
      cooldown: 25
    });
  }
  ...
  run(msg) {
    ...
    let prefix = msg.guildConfigs.prefix;
    ...
    if (poll[id]) return msg.sendMessage('**🚫 | There is already a poll running in this channel.**')
    if (!args[0]) return msg.sendMessage(`**To Create A Poll:**\n\`${prefix}poll <question> |<option1>|<option2>|<opt...\``);
    let split = args.join(' ').split('|');
    if (!split[0]) return msg.sendMessage(`*You must split your question and options with \`|\`\n**To Create A Poll:**\n\`${prefix}poll <question> |<option1>|<option2>|<opt...\``);
    if (!split[1]) return msg.sendMessage(`*You must have two or more options to make a poll*\n**To Create A Poll:**\n\`${prefix}poll <question> |<option1>|<option2>|<opt...\``);
    if (!split[2]) return msg.sendMessage(`*You must have two or more options to make a poll*\n**To Create A Poll:**\n\`${prefix}poll <question> |<option1>|<option2>|<opt...\``);
    poll[id] = {
      id,
      question: split[0],
      auth: msg.author.id,
    ...
    collect.on('collect', (m) => {
      ...
      if (m.content === 'endpoll') {
        ...
        m.reply(`✅ you've successfully voted.`);
      }
    });
    collect.on('end', (col, reas) => {
      if (reas === 'time' && col.size < 1) {
        msg.sendMessage('**The poll has ended without any votes.**');
        delete poll[id];
